...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if is_tty {
        match cli.words {
            Some(ref _x) => {
                let _ = play_tty(&cli, &mut answer_list, &mut guess_list);
                Ok(())
            }
            ...
            None => {
                ...
                loop {
                    turns_record += 1;
                    let success_flag = play_tty(&cli, &mut answer_list, &mut guess_list);
                    if success_flag > 0 {
                        success_record += 1;
        ...
        match cli.words {
            Some(ref _x) => {
                let _ = play_dis_tty(&cli, &mut answer_list, &mut guess_list);
                Ok(())
            }
            None => {
                let mut turns_record: i32 = 0;
                ...
                loop {
                    turns_record += 1;
                    let success_flag = play_dis_tty(&cli, &mut answer_list, &mut guess_list);
                    if success_flag > 0 {
                        success_record += 1;
                        try_record += success_flag;
                    }
