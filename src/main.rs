...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if is_tty {
        ...
        while turn<6{
            ...
            for i in 0..5{
                if !((guess_word_vector[i]>='A'&&guess_word_vector[i]<='Z')||(guess_word_vector[i]>='a'&&guess_word_vector[i]<='z')) {
                    println!("INVALID");
                    input_flag = false;
                    break;
                 }
            ...
            for i in 0..5{
                ...
                match s_status[i]{
                    ...
                    'G' => chracter_status[((guess_word_vector[i] as u8)-b'a' )as usize]='G',
                    'Y' if  chracter_status[((guess_word_vector[i] as u8)-b'a' )as usize]!='G' => chracter_status[((guess_word_vector[i] as u8)-b'a' )as usize]='Y',
                    'R' if (chracter_status[((guess_word_vector[i] as u8)-b'a' )as usize]!='G'&&chracter_status[((guess_word_vector[i] as u8)-b'a' )as usize]!='Y')=>{
            ...
            game_record.push(GameHistory { s_status_history: s_status.clone(), char_status_history: chracter_status.clone(), guesses: guess.clone()});
        ...
        if !flag { println!("Answer:{}",answer_word.to_uppercase());} 
        ...
        if flag{println!("CORRECT {}",turn);}
        else { println!("FAILED {}",answer_word.to_uppercase());} 
    }
    Ok(())
}
